...
from src.musicbert.hub_interface import MusicBERTHubInterface
        ...
        with open(os.path.join(self.args.data, 'label', split + ".label")) as file:
            for line in file:
                ...
                if len(label) < self.args.num_classes:
                    label = label + [self.label_dictionary.index('<pad>')] * (self.args.num_classes - len(label))
...
class MusicBERTSentencePredictionMultilabelCriterion(SentencePredictionCriterion):
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        if len(logging_outputs) > 0 and "y_pred" in logging_outputs[0]:
            y_pred = np.vstack(tuple(log.get("y_pred") for log in logging_outputs if "y_pred" in log))

            y_true = np.vstack(tuple(log.get("y_true") for log in logging_outputs if "y_true" in log))
...
class OctupleMaskTokensDataset(MaskTokensDataset):
    ...
    def __getitem__(self, index: int):
        with data_utils.numpy_seed(self.seed, self.epoch, index):
            ...
            def generate_mask(size, prob):
                mask_n = np.random.rand(size)
                mask_s = np.zeros(size, dtype=np.int8)
                mask_s += mask_n < prob * self.random_token_prob  # 3 -> random
                mask_s += mask_n < prob * (self.random_token_prob + self.leave_unmasked_prob)  # 2 -> original
            ...
            if strategy == 'bar':  # bar level mask
                mask[8: -8] = generate_mask((max_bars * max_instruments + len(self.vocab)) * 8, mask_prob) \
                    .reshape(-1, 8)[((item[8: -8: 8] - 4) * max_instruments) + (item[8 + 2: -8 + 2: 8] - 4)] \
                    .flatten()
...
@register_model("musicbert_composer")
class MusicBERTModel(RobertaModel):
    ...
    def from_pretrained(
        ...
        logger.info(x["args"])
        return MusicBERTHubInterface(x["args"], x["task"], x["models"][0])


@register_model_architecture("musicbert_composer", "musicbert_composer")
...
@register_model_architecture("musicbert_composer", "musicbert_base")
def musicbert_base_architecture(args):
    base_architecture(args)


@register_model_architecture("musicbert_composer", "musicbert_large")
...
@register_model_architecture("musicbert_composer", "musicbert_medium")
...
@register_model_architecture("musicbert_composer", "musicbert_small")
...
@register_model_architecture("musicbert_composer", "musicbert_mini")
...
@register_model_architecture("musicbert_composer", "musicbert_tiny")
