...
def gen_dictionary(file_name):
    ...
    with open(file_name, 'w') as f:
        for bar in range(bar_max):
            print('<0-{}>'.format(bar), num, file=f)
        for pos in range(beat_note_factor * max_notes_per_bar * pos_resolution):
            print('<1-{}>'.format(pos), num, file=f)
        for ins in range(max_inst + 1 + 1):
            # max_inst + 1 for percussion
            print('<2-{}>'.format(ins), num, file=f)
        for pitch in range(2 * max_pitch + 1 + 1):
            # max_pitch + 1 ~ 2 * max_pitch + 1 for percussion
            print('<3-{}>'.format(pitch), num, file=f)
        for dur in range(duration_max * pos_resolution):
            print('<4-{}>'.format(dur), num, file=f)
        for vel in range(v2e(max_velocity) + 1):
            print('<5-{}>'.format(vel), num, file=f)
        for ts in range(len(ts_list)):
            print('<6-{}>'.format(ts), num, file=f)
        for tempo in range(b2e(max_tempo) + 1):
            print('<7-{}>'.format(tempo), num, file=f)
...
def MIDI_to_encoding(midi_obj):
    ...
    # starting positions of each note
    ...
    pos_to_info: List = [[None for _ in range(4)] for _ in range(max_pos)]  # (Measure, TimeSig, Pos, Tempo)

    tsc = midi_obj.time_signature_changes
    tpc = midi_obj.tempo_changes

    # Set time signature and tempo for each time signature change and tempo change, or for each position
    ...
    # (aspil): Set defaults if above snippets didn't set TS and tempo (faster version of above snippet)
    if pos_to_info[:][1].count(None) == len(pos_to_info[1]):
        pos_to_info[:][1] = t2e(time_signature_reduce(4, 4))
    if pos_to_info[:][3].count(None) == len(pos_to_info[1]):
        pos_to_info[:][3] = b2e(120.0)
    cnt = 0
    bar = 0
    # Set bar number and position count in bar
    measure_length = None
    for j in range(len(pos_to_info)):
...
def encoding_to_MIDI(encoding):
    ...
    # make a list of lists where each nested list corresponds to a bar and each element holds the time signature
    # on that position of the bar
    bar_to_timesig: List = [list() for _ in range(max(map(lambda x: x[0], encoding)) + 1)]
    ...
    # bar_to_pos is a list where each element corresponds to the first position of each bar
    bar_to_pos: List = [None] * len(bar_to_timesig)
    cur_pos = 0
    for i in range(len(bar_to_pos)):
        bar_to_pos[i] = cur_pos
        ts = e2t(bar_to_timesig[i])  # current time signature
        measure_length = ts[0] * beat_note_factor * pos_resolution // ts[1]

        cur_pos += measure_length

    # pos_to_tempo is a list of lists where each nested list corresponds to a position and each element holds the
    # tempo on that position
    pos_to_tempo = [list() for _ in range(
        cur_pos + max(map(lambda x: x[1], encoding)))]

    # for each note, add the tempo to the list of tempos at that position
    for i in encoding:
...
def F(file_name):
    ...
    try:
        ...
        for p in range(0 - random.randint(0, sample_len_max - 1), len(e), sample_step):
            ...
            bar_index_list: List = [e[i][0] for i in range(L, R + 1) if e[i][0] is not None]
            ...
            bar_index_offset = random.randint(
                offset_lower_bound,
                offset_upper_bound) if offset_lower_bound <= offset_upper_bound else offset_lower_bound
            ...
            output_words = ['<s>'] * tokens_per_note + \
                           [('<{}-{}>'.format(j, k if j > 0 else k + bar_index_offset) if k is not None else '<unk>')
                            for i in e_segment for j, k in enumerate(i)] + \
                           ['</s>'] * (tokens_per_note - 1)  # tokens_per_note - 1 for append_eos functionality of
            # binarizer in fairseq
...
def G(file_name):
    try:
        ...
        print('ERROR(UNCAUGHT): ' + file_name + ' ' + str(e) + '\n', end='')
        return False
...
def encoding_to_str(e):
    ...
    return ' '.join((['<s>'] * tokens_per_note)
                    + ['<{}-{}>'.format(j, k if j > 0 else k + bar_index_offset) for i in e[p: p + sample_len_max]
                       if i[0] + bar_index_offset < bar_max for j, k in enumerate(i)]
                    + (['</s>'] * (tokens_per_note - 1)))  # 8 - 1 for append_eos functionality of binarizer in fairseq


if __name__ == '__main__':
    data_path = input('Dataset zip path: ')
    prefix = input('OctupleMIDI output path prefix: ')
    # data_path = 'data/GiantMIDI-Bach.zip'
    prefix = prefix + '_data_raw'
    # Create output directory for the Octuple encodings
    ...
    for sp in ['train', 'valid', 'test']:
        ...
        with Pool(pool_num) as pool:
            result = list(pool.imap_unordered(G, file_list_split))
